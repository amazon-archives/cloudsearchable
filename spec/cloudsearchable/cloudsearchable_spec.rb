...
describe Cloudsearchable do
  ...
  it 'can describe an index that returns ids for the class type' do
    ...
    expect(test_index).to be_kind_of(Cloudsearchable::Domain)
    expect(test_index.fields).to have(4).items #3 explicit + 1 for the id of the object
  end

  it 'has a default index' do
    expect(clazz.cloudsearch_index).to be_kind_of(Cloudsearchable::Domain)
    expect(clazz.cloudsearch_index(:test_index)).not_to be(clazz.cloudsearch_index)
  end

  it 'names domains consistent with CloudSearch limitations' do
    expect(clazz.cloudsearch_index(:test_index).name).to match /^[a-z][a-z0-9\-]+$/
  end
  ...
  describe 'an ordinary object' do
    ...
    it 'supplies the right values to the fields' do
      test_index = clazz.cloudsearch_index
      expect(test_index.fields[:test_class_id].value_for(inst)).to be(inst.id)
      expect(test_index.fields[:customer_id].value_for(inst)).to   be(inst.customer)
      expect(test_index.fields[:test_name].value_for(inst)).to     be(inst.name)
      expect(test_index.fields[:helpfulness].value_for(inst)).to   be(1234)
    end
    ...
    it 'generates a sensible addition sdf document' do
      ...
      expect(sdf[:fields][:helpfulness]).to be(1234)
    end
  end
